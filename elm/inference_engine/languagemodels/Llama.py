...
class Model(LanguageModel):
    ...
    def __init__(self, specs):
        ...
        if 'tokenizer_dir' not in specs.keys():
            ...
            self.raise_exception(error_message)
        if 'cache_dir' not in specs.keys():
            error_message = (
                f"cache_dir is a required input for this model family"
                        )
            self.raise_exception(error_message)
        ...
        self.cache_dir = specs['cache_dir']
    ...
    def load(self, quantization_config=None):
        ...
        self.quantization_config_used = bnb_config.to_dict() # for reproducibility logging to results file
